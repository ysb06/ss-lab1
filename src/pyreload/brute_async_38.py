...
async def measure_char_time(prefix, char) -> tuple[str, float]:
    ...
    # Python 3.8 호환성을 위해 asyncio.to_thread 대신 loop.run_in_executor 사용
    tasks = [loop.run_in_executor(None, get_time_from_run, test_guess) for _ in range(NOISE_SAMPLES)]
